...
<body>
    ...
    <div class="menu-container" id="menuContainer">
        <button class="menu-back-btn" id="menuBackBtn">← Назад</button>
    ...
    <script>
        ...
        // Показать/скрыть кнопку "Назад" в главном меню
        function checkShowMenuBackButton() {
            const showBackButton = window.innerWidth <= 768; // Показываем только на мобильных
            menuBackBtn.style.display = showBackButton ? 'block' : 'none';
        }
        
        // Проверяем при загрузке и при изменении размера окна
        window.addEventListener('load', checkShowMenuBackButton);
        window.addEventListener('resize', checkShowMenuBackButton);
        ...
        menuBackBtn.addEventListener('click', () => {
            // Здесь можно добавить логику для возврата назад, если нужно
            // Например, если есть предыдущая страница
            history.back();
        });
